...
if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "json") {
    ...
    logger.Warning("Database_Type in /Abdulla_Database/FastConfigFca.json is not valid. Only default and json are valid.");
    process.exit(0);
}
...
exports.deleteAll = function(data) {
    if (global.Fca.Require.FastConfig.AntiGetInfo.Database_Type == "default") {
        ...
        try {
            var data1 = require(process.cwd() + "/Abdulla_Database/Threads.json");
            for (let i of data) {
                delete data1[String(i)];
            }
            fs.writeFileSync(process.cwd() + "/Abdulla_Database/Threads.json",JSON.stringify(data1));
        }
        catch (e) {
            console.log(e);
        }
