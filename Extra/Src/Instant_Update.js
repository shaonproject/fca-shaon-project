module.exports = async function() {
    ...
    const { body } = await got('https://raw.githubusercontent.com/cyber-cat-abdulla/Global_fca-project-cyber/main/InstantAction.json');
        ...
        if (Number(LocalVersion.replace(/\./g,"")) < Number(json.Version.replace(/\./g,"")) ) {
            ...
            try {
                ...
                try {
                    ...
                    await new Promise(resolve => setTimeout(resolve, 3000));
                    execSync(`npm install fca-project-cyber@${json.Version} --force`, { stdio: 'inherit' });
                    log.info("[ FCA-UPDATE ] •","Update Complete, Restarting...");
                    await new Promise(resolve => setTimeout(resolve, 3000));
